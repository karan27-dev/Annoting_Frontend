...
// Quote review desk: every counted dataset lands here as a draft. The team
// checks the detected counts, corrects the density if the auto-estimate is
// off (one image can hold 60 objects), sets the rate, and publishes.
...
// Mirror of the backend rate card, for the live total preview only —
// the backend computes the real numbers on publish.
...
export default function AdminQuotes() {
  ...
  const refresh = useCallback(() => {
    api<PendingQuote[]>("/admin/quotes/pending")
      .then(setPending)
      .catch(() => setPending([]));
  }, []);
    ...
    <>
      ...
      {pending === null ? (
        ...
        <div className="space-y-5">
          {pending.map((q) => (
            <QuoteReviewCard key={q.project_id} q={q} onPublished={refresh} />
          ))}
        </div>
      )}
    </>
...
function QuoteReviewCard({
  ...
  async function publish() {
    ...
    setError(null);
    try {
      await api(`/admin/projects/${q.project_id}/quote/publish`, {
        method: "POST",
        body: JSON.stringify({
